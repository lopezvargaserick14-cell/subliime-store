        ...
        <>
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-charcoal/10">
              ...
              <button onClick={() => setIsCartOpen(false)} className="p-2 hover:bg-charcoal/5 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-charcoal/60 space-y-4">
                  ...
                  <button onClick={() => { setIsCartOpen(false); router.push('/catalog'); }}
                  ...
                  <div key={`${item.product.id}-${item.size}`} className="flex gap-4">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => removeFromCart(item.product.id, item.size)} className="text-xs text-charcoal/60 hover:text-charcoal underline">Eliminar</button>
                      </div>
                      <div className="flex justify-between items-end mt-4">
                        <div className="flex items-center border border-charcoal/20 rounded">
                          <button onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.size)} className="p-1.5 hover:bg-charcoal/5"><Minus size={14} /></button>
                          <span className="w-8 text-center text-sm">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.product.id, item.quantity + 1, item.size)} className="p-1.5 hover:bg-charcoal/5"><Plus size={14} /></button>
                        </div>
            ...
            {items.length > 0 && (
              <div className="p-6 bg-white border-t border-charcoal/10">
                ...
                <button onClick={() => { setIsCartOpen(false); router.push('/checkout'); }}
