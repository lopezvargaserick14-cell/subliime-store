...
export function CheckoutClient() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      window.location.href = `https://wa.me/573103068584?text=${encodeURIComponent(message)}`;
      clearCart();
    }, 800);
  };
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen pt-32 px-6 flex flex-col items-center justify-center text-center">
        ...
        <Link href="/catalog" className="uppercase tracking-widest text-sm border-b border-charcoal pb-1">Volver a la colección</Link>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div>
              ...
              <div className="space-y-4">
                ...
                />
                <input required type="email" value={formData.correo}
            ...
            <div className="payment-pill rounded-full p-4 flex flex-col md:flex-row items-center justify-end mt-8 gap-4">
              ...
              >
                ...
                <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" /></svg>
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="bg-stone-100 p-8">
            ...
            <div className="border-t border-charcoal/10 pt-6 space-y-4 text-sm">
              ...
              <div className="flex justify-between font-medium text-base pt-4 border-t border-charcoal/10">
                <span>Total</span><span className="font-serif text-xl">${cartTotal.toLocaleString('es-CO')}</span>
              </div>
            </div>
          </div>
        </div>
