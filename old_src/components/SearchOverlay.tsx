...
export function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) {
      // Navigate to catalog with search query, but since Catalog might not support it,
      // let's just do a basic implementation or route it.
      ...
      {isOpen && (
        ...
        >
          <div className="w-full max-w-4xl px-6 relative">
            <button 
              onClick={onClose}
              className="absolute right-6 top-0 p-2 opacity-50 hover:opacity-100 transition-opacity"
            >
              <X size={28} strokeWidth={1} />
            </button>
            <div className="flex flex-col items-center w-full">
              <p className="uppercase tracking-[0.2em] text-[10px] font-medium mb-6 opacity-60">
                Búsqueda
              </p>
            ...
            <div className="mt-12 h-[50vh] overflow-y-auto w-full no-scrollbar">
              {results.length > 0 ? (
                ...
                <p className="text-center italic opacity-50 font-serif pt-10">No se encontraron productos para "{query}"</p>
              ) : null}
            </div>
          </div>
        </motion.div>
